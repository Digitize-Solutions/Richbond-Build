...
export const Buttons = (props) => {
  ...
  return (
    <Wrapper className={`${className}-component`}>
      ...
      <ButtonsWrapper className={`${className}-content`} style={{ cursor: isPlayerLoading ? 'wait' : 'pointer' }}>
        {options.map((option, i) => {
          ...
          return (
            <ButtonWrapper
              ...
              style={{ pointerEvents: isPlayerLoading ? 'none' : 'all' }}
            >

              <div>{option.label}</div>

              <div id={option.label} style={{ textAlign: 'center', fontWeight: 'bold' }}> 0</div>
            </ButtonWrapper>
          );
        })}
      </ButtonsWrapper>
