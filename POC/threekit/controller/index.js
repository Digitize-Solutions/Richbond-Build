...
class Controller {
  ...
  getBom() {
    const attributes = window.threekit.configurator.getDisplayAttributes({ includeHidden: true });
  ...
  uploadImage(blob, name) {
    return new Promise(async (resolve) => {
      const [
        uploadResponse,
        uploadError,
      ] = await threekitAPI.catalog.uploadAsset({ name, blob });
      if (uploadError) resolve(undefined);

      const { jobId } = uploadResponse.data[0];

      const [
        assetId,
        assetIdError,
      ] = await threekitAPI.catalog.getUploadAssetId(jobId);
      if (assetIdError) resolve(undefined);
      resolve(assetId);
    });
  }
}
